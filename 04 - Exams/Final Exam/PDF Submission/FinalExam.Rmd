...
![](Images/problem2i.png){ height=80% }

\newpage
## *Part ii*: Path Relinking
* Below shows the path re-linking overview.
* Shows each  iterations that occurred in order to find the guided neighbor
* Green arrow highlights the, best solutions found along the path
* **Logic overview:** Evaluates neighbors  from initiation solution, then makes the best move. Updates the difference between current and guided solution. Repeats until finding the guided solution
* *`Iteration 3` duplicated for ease of viewing*

![](Images/problem2ii_1.png)

![](Images/problem2ii_2.png)

\newpage
## *Part iii*: Simulated Annealing
* Current temperature = 3

Using the evaluation function for this questions
```{r}
# Evaluation function
...
is the candidate solution since minimization.  
...
Now calculate the probability from the current to the candidate solutions  
```{r}
