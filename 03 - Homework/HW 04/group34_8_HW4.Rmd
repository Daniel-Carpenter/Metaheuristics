...
q4 = kRestartsHillClimbFirstAccept(k_restarts, numSolutionsToShow)
```
...
## HC w. Random Walk, using *First Acceptance*
...
# Get the python question list from above
pyList <- py$output
...
rownames(df) <- c('Local Search with Best Improvement',
                  ...
                  'Local Search with Random Walk, using First Acceptence'
                  )
```
