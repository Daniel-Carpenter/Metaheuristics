...
## Functions for the Global and Local Best Logic (*using `Ring Structure`*)
...
* Allows you to easily change the swarm size, dimensions, an the local or global best method
...
## Function to Calculate Particle Historical Best Fitness Value and Position
...
## 2 Functions to Display Best Solution and Write to CSV
...
## Driver Funtion to Compile and Loop through a PSO Problem
...
### `1(b) i. - iii.` Iteration Table & Plot
...
```{r thePlot, cache=FALSE, fig.height=9}
require(ggplot2)
...
### 1(c) 200D Global Swarm w/Various Param
...
* My computer cannot handle such large loads. It is likely that higher number of iterations 
would lead to closer to the known optima
...
### 1(d) 2D Local Swarm
*Implement a PSO algorithm that uses the `local best` with `ring` topology in place of the global best. See aboce logic and explanation*
...
### 1(e) 200D Local Swarm w/Various Params.
